...
public class DataModelServiceTest {
    ...
    void uploadFileUnauthorized() {
        assertThrows(UnauthorizedException.class, () -> dataModelService.uploadFile(TOKEN_1, FILENAME_1, MULTIPART_FILE));
    }
    ...
    void deleteFileUnauthorized() {
        assertThrows(UnauthorizedException.class, () -> dataModelService.deleteFile(TOKEN_1, FILENAME_1));
    }
    ...
    void downloadFileUnauthorized() {
        Mockito.when(dataModelRepository.findByUserAndFilename(USER_1, FILENAME_1)).thenReturn(DATA_MODEL_1);
        assertThrows(UnauthorizedException.class, () -> dataModelService.downloadFile(TOKEN_1, FILENAME_1));
    }
    ...
    void editFileNameUnauthorized() {
        assertThrows(UnauthorizedException.class, () -> dataModelService.editFileName(TOKEN_1, FILENAME_1, EDIT_FILE_NAME_REQUEST));
    }
    ...
    void getAllFilesUnauthorized() {
        Mockito.when(dataModelRepository.findAllByUser(USER_1)).thenReturn(DATA_MODEL_LIST);
        assertThrows(UnauthorizedException.class, () -> dataModelService.getAllFiles(TOKEN_1, LIMIT));
    }
}
